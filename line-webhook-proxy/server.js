...
if (!GAS_BASE_URL) {
  console.warn('Missing GAS_BASE_URL');
}
...
async function processWebhookPayload(payload) {
  ...
  for (const event of events) {
    ...
    if (!userId) {
      console.warn('Skip event because no userId is available');
      continue;
    }
    ...
    try {
      ...
      await pushLineMessage(userId, flex);
      console.log(`Pushed digest for ${dateStr} to ${userId}`);
    } catch (error) {
      ...
      await pushLineMessage(userId, {
        ...
        console.error('[push error fallback]', pushError && pushError.stack ? pushError.stack : pushError);
      });
    }
  }
}
...
function parseQueryDate(text) {
  ...
  if (value === '今天') return formatMdDate(today);
  if (value === '明天') {
...
function buildFlexMessage(digest) {
  ...
  if (periods.length) {
    ...
    tableRows.push({
      ...
      layout: 'vertical',
      paddingAll: '8px',
      contents: [{ type: 'text', text: '（當日無課程進度）', size: 'lg', color: '#9aaabb', align: 'center' }]
    });
  }
  ...
  const contactLines = contactText ? contactText.split('\n').map((line) => line.trim()).filter(Boolean) : [];
  if (contactLines.length) {
    for (const line of contactLines) {
      footerItems.push({ type: 'text', text: line, size: 'lg', color: '#50627f', wrap: true, margin: 'xs' });
    }
  } else {
    footerItems.push({ type: 'text', text: '（當日無聯絡簿內容）', size: 'lg', color: '#aaaaaa' });
  }
  ...
  return {
    type: 'flex',
    altText: `${digest.dayLabel || digest.date || ''} 教學進度`,
    quickReply: {
      items: [
...
function getRowColor(period) {
  ...
  const homeroomKeywords = ['國語', '數學', '社會', '健康', '樂活'];
  const subjectKeywords = ['自然', '藝專', '閩語', '視覺', '英語', '分部課', '樂理'];
  if (homeroomKeywords.some((keyword) => subject.includes(keyword) || content.includes(keyword))) return '#f5f0eb';
  if (subjectKeywords.some((keyword) => subject.includes(keyword) || content.includes(keyword))) return '#dde6dc';
  return '#ffffff';
}
